...
pub fn send_eoi() {
    if is_active() {
        // Létrehozunk egy ideiglenes LocalApic-ot a küldéshez
...
pub unsafe fn init() {
    // 1. Régi PIC lekapcsolása
    {
        let mut pics = PICS.lock();
        pics.disable_pic();
    }

    // 2. Local APIC inicializálása
    let lapic = LocalApic::new(LAPIC_DEFAULT_BASE);
    lapic.init();

    // 3. I/O APIC inicializálása
    let ioapic = IoApic::new(0xFEC00000);
    ioapic.init();

    ioapic.set_irq(0, 32);
    ioapic.set_irq(1, 33);
    // IRQ 12 az egérnek, ha később kell
    ioapic.set_irq(12, 44);

    // Jelezzük az IDT-nek is, hogy váltson APIC módba az EOI-nál
    crate::arch::idt::set_apic_mode(true);
    
    // Globális flag beállítása
    APIC_ENABLED = true;
